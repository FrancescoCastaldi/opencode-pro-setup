...
function Step-Verify {
    ...
    # Skills: list all discovered skill directories and highlight GitHub management skills
    $skillDirs = @(Get-ChildItem -Path (Join-Path $CONFIG_DIR "skills") -Directory -ErrorAction SilentlyContinue)
    $skillNames = $skillDirs | ForEach-Object { $_.Name }
    $githubSkills = @('github', 'git-release', 'git-workflow-manager', 'using-git-worktrees', 'github-issue-triage', 'github-repo-optimizer')
    $foundGithubSkills = $skillNames | Where-Object { $githubSkills -contains $_ }

    if ($skillDirs.Count -gt 0) {
        $report += "Skills: $($skillDirs.Count)"
        $report += "  - GitHub skills: $($foundGithubSkills -join ', ')"
        foreach ($name in $skillNames) {
            if ($githubSkills -notcontains $name) {
                $report += "  - $name"
            }
        }
    } else {
